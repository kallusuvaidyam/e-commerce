<template>
  <nav class="bg-gray-500 flex justify-between items-center px-5 py-6 h-[65px] fixed w-full z-19">
    <RouterLink to="/" class="rounded-full p-1 border hover:border-2 transition all duration-300 ease-in-out">
      <img src="../../images/suvaidyam_bg_remove.png" alt="Suvaidyam logo"
        class="h-[40px] w-[40px] rounded move circle" />
    </RouterLink>

    <div id="setting_bar" class="flex">
      <ul :class="['md:flex gap-5 text-[18px]', menuOpen ? 'show' : '']">
        <li v-for="(item, i) in menuItems" :key="i"
          class="text-white px-6 py-1 rounded transition-all duration-300 ease-in-out group">
          <RouterLink :to="item.link" @click="menuOpen = false">{{ item.name }}</RouterLink>
          <span
            class="block h-[1px] w-full bg-white scale-x-0 group-hover:scale-x-100 transition-transform duration-300 origin-left"
            :class="[router.path === item.link ? 'scale-x-100' : 'scale-x-0']"> </span>
        </li>
      </ul>
...
const menuItems = [
  ...
  { name: 'Blog', link: '/blog' },
  { name: 'Learn more', link: '/learn_more' },
]
